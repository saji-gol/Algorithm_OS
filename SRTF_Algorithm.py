...
def Gantt_Chart(Name, ProcessInfo, CS):
    ...
    while (1):
        ...
        TurnIndex = Result[0]
        MR = Result[1]
        if TurnIndex not in seen:
            seen.append(TurnIndex)
            StartFinishProcess[TurnIndex][0] = CurentTime

        tmp_CurrentTime = CurentTime
        tmp_ProcessInfo = ProcessInfo
        while (1):
            tmp_ProcessInfo[TurnIndex][2] = tmp_ProcessInfo[TurnIndex][2] - 1
            tmp_CurrentTime += 1
            tmp_Result = ProcessTurn(tmp_ProcessInfo, tmp_CurrentTime)
            tmp_TurnIndex = tmp_Result[0]

            if tmp_TurnIndex != TurnIndex:
                CurentTime = tmp_CurrentTime
                break
...
if A == 2:
    ...
    while(WS == 0):
        ...
        if(name == '1'):
            os.system('py .\SRTF_Algorithm.py')
    ...
    ListProcess = [
        ...
        [['A', 'B', 'C', 'D'], [[0, 8, 8], [1, 8, 8], [2, 2, 2], [2, 1, 1]], [0]]
    ]
